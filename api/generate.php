...
<div class="card">
  ...
  <div style="background:#FAEEDA;border-radius:10px;padding:10px 14px;margin-bottom:1rem;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
    <div>
      ...
      <div style="font-size:14px;font-weight:500;color:#633806;">
        <?php echo $loss_kg; ?>კგ-ის დაკლება <?php echo $timeline_weeks; ?> კვირაში
      </div>
  ...
  <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:<?php echo (!empty($profile['allergies']) || !empty($profile['health_notes'])) ? '10px' : '0'; ?>;">
    <span style="background:#F1EFE8;border-radius:99px;padding:4px 12px;font-size:12px;color:#444441;">
      <?php echo isset($goal_labels[$profile['goal']]) ? $goal_labels[$profile['goal']] : $profile['goal']; ?>
    </span>
    <span style="background:#F1EFE8;border-radius:99px;padding:4px 12px;font-size:12px;color:#444441;">
      <?php echo isset($act_labels[$profile['activity_level']]) ? $act_labels[$profile['activity_level']] : $profile['activity_level']; ?>
    </span>
    <span style="background:#F1EFE8;border-radius:99px;padding:4px 12px;font-size:12px;color:#444441;">
      ბიუჯეტი: <?php echo isset($bud_labels[$profile['budget']]) ? $bud_labels[$profile['budget']] : $profile['budget']; ?>
    </span>
    <span style="background:#F1EFE8;border-radius:99px;padding:4px 12px;font-size:12px;color:#444441;">
      <?php echo $profile['age']; ?> წ. · <?php echo $profile['gender'] === 'male' ? '♂' : '♀'; ?>
    </span>
  </div>

  <?php if (!empty($profile['allergies'])): ?>
  <div style="background:#FCEBEB;border-radius:8px;padding:8px 12px;font-size:12px;color:#A32D2D;margin-bottom:6px;">
    ⚠️ ალერგია/შეზღუდვა: <?php echo sanitize($profile['allergies']); ?>
  </div>
  <?php endif; ?>

  <?php if (!empty($profile['health_notes'])): ?>
  <div style="background:#E6F1FB;border-radius:8px;padding:8px 12px;font-size:12px;color:#185FA5;">
    🏥 სამედიცინო: <?php echo sanitize($profile['health_notes']); ?>
  </div>
  <?php endif; ?>
...
<div id="progress-wrap" style="display:none;margin-top:1.5rem;">
  <div class="card" style="text-align:center;padding:2rem;">
    <div id="progress-icon" style="margin-bottom:1rem;">
      <div style="width:40px;height:40px;border:3px solid #E1F5EE;border-top-color:#1D9E75;border-radius:50%;animation:spin .8s linear infinite;margin:0 auto;"></div>
    </div>
    <div id="progress-text" style="font-size:15px;font-weight:500;color:#1A1A18;margin-bottom:.5rem;">AI მუშაობს...</div>
    <div id="progress-sub" style="font-size:13px;color:#888780;">გეგმა გენერირდება, 20-40 წამი სჭირდება</div>
...
var messages = [
  'AI მუშაობს...',
  'ქართული ბაზრის ფასები...',
  'კალორიების გამოთვლა...',
  'მენიუს შედგენა...',
  'გეგმა თითქმის მზადაა...'
];
var msgIdx = 0;
...
function startGeneration() {
  ...
  // Single step: create_job returns job_id, runs AI in background via flush
  ...
  xhr.onload = function() {
    ...
    try { data = JSON.parse(xhr.responseText); } catch(e) {
      ...
      showError('შეცდომა: ' + xhr.responseText.substring(0, 150)); return;
    }
